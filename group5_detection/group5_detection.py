...
def segment_bb_frustum_from_projected_pcd(bb_list, pts_2d, projected_grid, calib, labels=[], visualize=True, masks = None, orig_img = None):
    """
    ...
    def segmentation_job(i, idx, pcd_list, visualize=False):        
        if masks is not None:
            mask_inds = np.where(((masks[i][:, :] == True) & ((projected_grid[:,:,0] > 0) | (projected_grid[:,:,1] > 0) | (projected_grid[:,:,2] > 0))))
    ...
    pcd_list = [] * len(inds)
    o3d_pcd_list = [] * len(inds)
    mask_points_list = []

    # Non parallel
    for i, idx in enumerate(inds): 
        segmentation_job(i, idx, pcd_list)
    
    if visualize:
        for velo_points in pcd_list:
            pcd = open3d.geometry.PointCloud()
            pcd.points = open3d.utility.Vector3dVector(np.transpose(velo_points))
...
def apply_dbscan(pointcloud, keep_n=None, visualize=True):
    ...
    # Run DBSCAN on point cloud
    # with open3d.utility.VerbosityContextManager(open3d.utility.VerbosityLevel.Debug) as cm:
    labels = np.array(pcd.cluster_dbscan(eps=.5, min_points=5))
...
def mask_rcnn_detection_analysis(generated_3d_bb_list, kitti_gt_3d_bb):
    ...
    for i, kitti_3d_bb in enumerate(kitti_gt_3d_bb):
        ...
        generated_3d_corners = np.asarray(closest_generated_box.get_box_points())
        ...
        (IOU_3d,IOU_2d)=box3d_iou(generated_3d_corners,kitti_gt_corners)
        ...
        print(f'{"3D IoU: ":<50} {IOU_3d:>10.4f}')
        print(f'{"2D IoU: ":<50} {IOU_2d:>10.4f}')
        print('.'*50)
...
def mask_detection(file_num = 0, use_vis = False):
    ...
    avg_scene_time = 0
    max_scene_time = -float('inf')

    total_detections = 0
    min_individual_detection_time = float('inf')
    avg_individual_detection_time = 0
    max_individual_detection_time = -float('inf')
    ...
    for i, file in enumerate(run_files):
        ...
        # ground_remove_kitti_gt_pcd = kitti_gt_pointclouds
        time_diff= time.perf_counter() - start
        ...
        #############################################################################
        start = time.perf_counter()
        projected_kitti_gt_pcd, projected_kitti_grid = render_lidar_on_image(ground_remove_kitti_gt_pcd, kitti_gt_images, kitti_gt_calibs, kitti_gt_images.shape[1], kitti_gt_images.shape[0], visualize=use_vis)
        time_diff= time.perf_counter() - start
        total_time += time_diff
        ...
        #############################################################################
        start = time.perf_counter()
        frustum_segmented_kitti_gt_pcd_list = segment_bb_frustum_from_projected_pcd(mr_inf_2d_bb_list, projected_kitti_gt_pcd, projected_kitti_grid, kitti_gt_calibs, labels=kitti_gt_3d_bb, masks=mr_inf_segmentations, visualize=use_vis, orig_img=kitti_gt_images)
        time_diff= time.perf_counter() - start
        total_time += time_diff
        ...
        if total_time > max_scene_time:
            max_scene_time = total_time
        if total_time < min_scene_time:
            min_scene_time = total_time
        avg_scene_time += total_time
        print(f'File {str(file).zfill(6)} Object Detection {"Total Execution Time":<40}: {total_time:.>5.4f} s.')
        
        avg_time_per_detection = total_time/len(mr_inf_labels)
        if avg_time_per_detection > max_individual_detection_time:
            max_individual_detection_time = avg_time_per_detection
        if avg_time_per_detection < min_individual_detection_time:
            min_individual_detection_time = avg_time_per_detection
        avg_individual_detection_time += avg_time_per_detection
        total_detections += len(mr_inf_labels)
        print(f'File {str(file).zfill(6)} Object Detection {"Avg Inference Time/Detection":<40}: {avg_time_per_detection:.>5.4f} s.')
    ...
    print('='*50)
    print(f'3D Object Detection {"Min Scene Inference Time":<40}: {min_scene_time:.>5.4f} s.')
    print(f'3D Object Detection {"Avg Scene Inference Time":<40}: {avg_scene_time/len(test_list):.>5.4f} s.')
    print(f'3D Object Detection {"Max Scene Inference Time":<40}: {max_scene_time:.>5.4f} s.')
    print(f'3D Object Detection {"Min Inference Time Per Detection":<40}: {min_individual_detection_time:.>5.4f} s.')
    print(f'3D Object Detection {"Avg Inference Time Per Detection":<40}: {avg_individual_detection_time/total_detections:.>5.4f} s.')
    print(f'3D Object Detection {"Max Inference Time Per Detection":<40}: {max_individual_detection_time:.>5.4f} s.')
...
def group5_call():
    ...
    #############################################################################
    start = time.perf_counter()
    points, projected_grid = render_lidar_on_image(segmented_pc_velo, rgb, calib, img_width, img_height, visualize=False)
    time_diff= time.perf_counter() - start
    total_time += time_diff
    print(f'{"3D to 2D Projection Time":<30}: {time_diff:.>5.4f} s.')
    ...
    start = time.perf_counter()
    frustum_pcd_list = segment_bb_frustum_from_projected_pcd(detector_bb_list, points, projected_grid, calib, labels=get_groundtruth_3d_bb(labels, calib), masks=mask_segm, visualize=True, orig_img=orig_img)
    time_diff= time.perf_counter() - start
    total_time += time_diff
    print(f'{"Frustum Segmentation Time":<30}: {time_diff:.>5.4f} s.')
...
if __name__ == '__main__':
    ...
    mask_detection(test_list, False)
